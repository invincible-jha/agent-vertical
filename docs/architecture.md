# Architecture — agent-vertical

## Overview

Domain-specific agent templates for common enterprise use cases

This document describes the high-level architecture of agent-vertical
...
agent-vertical uses a decorator-based plugin registry backed by
``importlib.metadata`` entry-points. This allows third-party packages
(including the AgentVertical enterprise edition) to extend the system
without modifying the core.

### Registration at import time

```python
from agent_vertical.plugins.registry import PluginRegistry
from agent_vertical.core import BaseProcessor  # example base class
