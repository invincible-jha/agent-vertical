...
```bash
pip install "agent-vertical[agentcore]"
```

### 2. Swap the configuration backend

Replace the default ``Settings`` class with the enterprise version
which reads from your AgentVertical control plane:

```python
# Before (open-source)
from agent_vertical.core import Settings
...
The AgentVertical SDK ships with a full set of enterprise plugins that
auto-register when the package is installed:

```python
from agent_vertical.plugins.registry import processor_registry
processor_registry.load_entrypoints("agent_vertical.plugins")
```
